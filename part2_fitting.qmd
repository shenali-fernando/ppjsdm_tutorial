...
If parameters are misspecified in the fit you will see this warning message, and/or the interaction coefficient (alpha) matrix will be NULL (for a quick check run `coef(fit)` or `fit$coefficients$alpha` for this). 
...
This error occurs when the short-range radii parameter is missepecified. Specifically, when the matrix of radii values has number of rows and columns that does not match the number of groups (types) included in the model.
...
As mentioned previously, the model does not work well if you have binary covariates. It is possible to smooth covariates so they are no longer binary. However, if you must use binary covariates in the model, call parameter [use_regularisation = TRUE]{.underline} when fitting the model with 'gibbsm'.
...
The use_regularisation (default set to FALSE) allows for lasso regularisation to occur in to the regression model (when computing the variance-covariance matrix). This parameter can also be use when wanting to shrink coefficients towards zero when there are many large coefficients occurring in the model, and will shift coefficients towards 0.
