...
There are a wide variety of plots that can be created. You do not need to keep to the functions built into the package. To create other plots, we need to change the summary output into a dataframe. This function works inside the visualisation functions above, but we can extract it here as its own function:
